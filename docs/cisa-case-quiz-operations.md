...
- Buka `docs/cisa-domain-4-5-case-quiz.html` langsung di Chrome/Edge; halaman tidak mengambil data atau library dari internet. Narasi, pertanyaan, dan bukti tiap paket disimpan sebagai ciphertext AES-256-GCM dengan kunci yang diturunkan dari sandi melalui PBKDF2-SHA-256. Konten baru dimasukkan ke halaman setelah sandi berhasil mendekripsi paket.
...
- Sandi paket tidak disimpan sebagai teks atau hash pembanding di HTML. Namun, karena dekripsi terjadi pada perangkat mahasiswa, isi dapat dilihat atau disalin setelah sandi dibagikan dan paket berhasil dibuka. Enkripsi ini melindungi soal sebelum sandi diberikan, bukan DRM setelah pembukaan.
- Sumber soal tanpa enkripsi dan dua sandi disimpan hanya pada file lokal yang masuk `.gitignore`. Setiap perubahan soal perlu diikuti `node scripts/build-cisa-worksheet.cjs` untuk menghasilkan kembali HTML distribusi.
