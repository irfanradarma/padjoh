...
df['Query SQL', 'SQL Drill', 'DBase Breach', 'Payroll', 'Travel Expense', 'Average'] = df['Query SQL', 'SQL Drill', 'DBase Breach', 'Payroll', 'Travel Expense', 'Average'].applymap(lambda x: f"{x:.2f}" if pd.notnull(x) else x)
