...
Deno.serve(async (req) => {
  ...
  try {
    const { rows, rubric, domain } = await req.json()

    const anthropic = new Anthropic({ apiKey: Deno.env.get('ANTHROPIC_API_KEY') })

    const rowsText = (rows ?? [])
      .filter((r: Record<string, string>) => r.level1 || r.level2 || r.level3)
      .slice(0, 200)
      .map((r: Record<string, string>) => {
        const path = ['level1', 'level2', 'level3', 'level4', 'level5']
          .map(l => r[l]).filter(Boolean).join(' → ')
        return r.summary ? `${path}  [${r.summary}]` : path
      })
      .join('\n')

    const maxTotal = rubric.criteria.reduce((s: number, c: { max_score: number }) => s + Number(c.max_score || 0), 0)
    ...
    const prompt = `Kamu adalah penilai akademis untuk mata kuliah IS Audit di PKN STAN.
Nilailah mind map mahasiswa berikut untuk Domain ${domain} menggunakan rubrik yang diberikan.

RUBRIK (Total maks ${maxTotal} poin):
${rubricText}

DATA MIND MAP:
${rowsText || '(Tidak ada data)'}
